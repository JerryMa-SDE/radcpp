...
bool VulkanShader::LoadBinary(VkShaderStageFlagBits stage, void* binary, size_t binarySize)
{
    ...
    {
        LogVulkan(Warn, "VulkanShader::LoadBinary: %p: magic number doesn't match 0x07230203.",
            binary);
    }
    m_stage = stage;
    return true;
...
bool VulkanShader::LoadBinaryFromFile(const std::string_view fileName, VkShaderStageFlagBits stage)
{
    rad::File binaryFile;
    if (binaryFile.Open((char8_t*)fileName.data(), rad::FileAccessRead | rad::FileAccessBinary))
    {
        uint64_t fileSize = binaryFile.GetSize();
        if (fileSize % sizeof(uint32_t) != 0)
        {
            LogVulkan(Error, "VulkanShader::LoadBinaryFromFile: %s: the binary is invalid: size=%llu (should be mutliple of uint32).",
        ...
        if (m_binary[0] != 0x07230203)
        {
            LogVulkan(Error, "VulkanShader::LoadBinaryFromFile: %s: the binary is invalid: magic number doesn't match 0x07230203.",
                fileName.data());
        }
        m_stage = stage;
        m_fileName = fileName;
...
{
    std::string buffer;
    size_t bufferSize = (m_binary.size() + 8) * 120;
    bufferSize = rad::Pow2AlignUp(bufferSize, 1024);
    bufferSize = std::min<size_t>(bufferSize, 4 * 1024 * 1024);
    ...
    {
        LogVulkan(Warn, "VulkanShader::Disassemble: %s", buffer.c_str());
        return std::string();
    }
}
