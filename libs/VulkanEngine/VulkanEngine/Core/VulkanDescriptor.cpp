...
void VulkanDescriptorSet::UpdateBuffers(
    uint32_t binding, uint32_t arrayElement, VkDescriptorType type, rad::Span<VulkanBuffer*> buffers)
{
...
void VulkanDescriptorSet::UpdateUniformBuffers(
    uint32_t binding, uint32_t arrayElement, rad::Span<VulkanBuffer*> buffers)
{
    UpdateBuffers(binding, arrayElement, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, buffers);
}
