...
public:
    ...
    rad::Ref<VulkanImage> CreateImage(const VkImageCreateInfo& createInfo, VmaMemoryUsage memoryUsage = VMA_MEMORY_USAGE_AUTO);
    ...
    rad::Ref<VulkanImage> CreateImage2DTexture(
        VkFormat format, uint32_t width, uint32_t height, uint32_t mipLevels, VkImageUsageFlags addUsage = 0);
